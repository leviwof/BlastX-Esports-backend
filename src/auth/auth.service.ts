...
export class AuthService {
  ...
  private hash(otp: string): string { const pepper = this.config.get<string>('OTP_PEPPER') || 'blastix-default-otp-pepper-secret'; return createHash('sha256').update(`${otp}:${pepper}`).digest('hex'); }
